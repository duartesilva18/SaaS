    ...
    if len(monthly_data) > 0:
        ...
        if projected_balance < 0 and len(insights) < 3:
            ...
            if months_until_negative > 0:
                insights.append(schemas.InsightItem(
                    ...
                    message=f"Se mantiveres este ritmo, o teu saldo ficará negativo em {months_until_negative} mês(es). Ação urgente necessária.",
                    icon='trending-down',
                ...
                insights.append(schemas.InsightItem(
                    type='warning',
                    title='⚠️ PROJEÇÃO NEGATIVA',
                    message=f"Projeção indica saldo negativo nos próximos 3 meses. Reavalia os teus gastos.",
                    icon='alert-triangle',
                    trend='down'
                ))
        ...
        if categories_at_risk and len(insights) < 3:
            ...
            if top_risk[1] >= 100:
                insights.append(schemas.InsightItem(
                    type='danger',
                    title='🎯 RISCO DE EXCEDER LIMITE',
                    message=f"{top_risk[0]} pode exceder o limite ({top_risk[3]:.0f}€) no próximo mês. Projeção: {top_risk[2]:.0f}€.",
                    icon='target',
                    value=top_risk[1],
                    trend='up'
                ))
                ...
                insights.append(schemas.InsightItem(
                    ...
                    message=f"{top_risk[0]} está a aproximar-se do limite ({top_risk[3]:.0f}€). Projeção: {top_risk[2]:.0f}€.",
                    icon='alert-circle',
                    value=top_risk[1],
                    trend='up'
                ))
            ...
            if current_vault_total < emergency_goal and monthly_vault_contribution > 0:
                ...
                if months_to_goal <= 12 and len(insights) < 3:
                    insights.append(schemas.InsightItem(
                        type='success',
                        title='🎯 META EM VISTA',
                        message=f"Com este ritmo, atinges 6 meses de fundo de emergência em {months_to_goal:.0f} mês(es). Mantém o foco!",
                        icon='target',
            ...
            if current_vault_total < full_emergency_goal and monthly_vault_contribution > 0:
                ...
                if 12 < months_to_full_goal <= 24 and len(insights) < 3:
                    insights.append(schemas.InsightItem(
                        ...
                        message=f"A caminho de 12 meses de fundo de emergência em {months_to_full_goal:.0f} mês(es).",
                        icon='target',
        ...
        if len(monthly_data) >= 3:
            ...
            if abs(seasonal_change) > 15 and len(insights) < 3:
                if seasonal_change > 0:
                    insights.append(schemas.InsightItem(
                        type='warning',
                        title='📊 PADRÃO SAZONAL DETETADO',
                        message=f"Os teus gastos aumentaram {seasonal_change:.0f}% nos últimos meses. Pode ser um padrão sazonal.",
                        icon='trending-up',
                    ...
                    insights.append(schemas.InsightItem(
                        ...
                        message=f"Os teus gastos diminuíram {abs(seasonal_change):.0f}% nos últimos meses. Excelente progresso!",
                        icon='trending-down',
